...
master_box_value_coder = ABIType.from_string('(uint64,byte[])')
...
for master_box in all_master_boxes:
    ...
    if master_box_name_raw[:1] != b'a':
        ...
        petition_app_id, petition_text_ints = algorand.app.get_box_value_from_abi_type(app_id=master_petition_app_id, box_name=master_box_name_raw, abi_type=master_box_value_coder)
        petition_text = bytes(petition_text_ints).decode()
        print(f'Petition App ID: {petition_app_id}, Petition ID: {petition_id} Petition Text: {petition_text}')
        try:
            additional_text = algorand.app.get_box_value(app_id=master_petition_app_id, box_name=b'a' + master_box_name_raw)
            # print(f'Additional Text: {len(additional_text)} characters, {additional_text}')
            print(f'Had additional text')
        except:
        ...
        for petition_box in petition_boxes:
            ...
            if len(petition_box_name_raw) == 32:
                ...
                print(f'Found Signature: {signee} Signed at {time_of_signature}')
